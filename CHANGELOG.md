...
- **Лимит трафика у клиента AmneziaWG и у панельного пользователя (#95).**
  ...
  `used_tx`, `used_reset_at` и `suspend_reason`. Сохранение, которое пока не
  вступило в силу (висит черновик конфига или конфиг только для чтения),
  возвращается с полем `warning`, называющим причину.

  **Известное ограничение.** Отсечка пира на sing-box-бэкенде переписывает
  активный конфиг, а это молча откладывается, пока висит черновик правок. Пир,
  исчерпавший квоту (или истёкший) в такой момент, показан в панели как
  приостановленный, но продолжает ходить в сеть до ближайшего Apply —
  интерактивные операции с квотой в это время отвечают 409 и прямо просят
  применить или отменить черновик. У панельных пользователей та же отсрочка
