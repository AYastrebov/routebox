...
- **Лимит трафика у клиента AmneziaWG и у панельного пользователя (#95).**
  У пира и у пользователя появился разовый лимит на сумму принятого и отданного:
  в форме создания клиента и в ряду «Квота» на странице AmneziaWG, в строке
  пользователя рядом со сроком. Вводится в гигабайтах (1024³, дроби допустимы),
  на диск уходит числом байт. Исчерпание приостанавливает тем же механизмом, что
  и истечение срока: пир снимается с интерфейса, пользователь попадает в
  reject-маршрут по `auth_user` — секреты остаются на месте, и вернуть в строй
  можно, подняв лимит или нажав «Сбросить счётчик»; срок и квота независимы,
  продление даты не воскрешает исчерпавшего лимит. Причина приостановки в строке
  одна, по приоритету «вручную → квота → срок». Счётчики накопительные и лежат
  рядом с самой сущностью (`peers.toml`, `users.toml`), а не в истории трафика:
  они переживают перезапуск панели, амнезия-бокса и очистку SQLite, а первый
  снимок за жизнь процесса только берёт опору, поэтому перезапуск панели под
  живым туннелем не засчитывает трафик повторно. Пополняются они тиком в 30 с —
  так и написано в подсказке к полю, — из-за чего небольшой перебор над лимитом
  нормален. Резервная копия AmneziaWG (`GET /api/awg/backup`) несёт лимит и
  счётчики как есть, так что восстановление на другом хосте продолжает счёт, а
  не выдаёт всем новую квоту. Подписка отдаёт лимит в заголовке
  `Subscription-Userinfo: total=`, а `upload=`/`download=` рядом с ним — те же
  счётчики квоты. Точки API: `quota_bytes` в `PATCH /api/awg/peers/{pk}/expiry`
  и `PATCH /api/users/{id}`, новые `POST /api/awg/peers/{pk}/traffic/reset` и
  `POST /api/users/{id}/traffic/reset`; списки отдают `quota_bytes`, `used_rx`,
  `used_tx`, `used_reset_at` и `suspend_reason`. У панельного пользователя
  (`PATCH /api/users/{id}` и его сброс счётчика) сохранение, которое пока не
  вступило в силу — висит черновик конфига или конфиг только для чтения, —
  возвращается с полем `warning`, называющим причину; у пира соответствующие
  ручки в этот момент вообще ничего не сохраняют, а отвечают 409.

  **Известное ограничение.** Отсечка пира на sing-box-бэкенде переписывает
  активный конфиг, и в двух случаях до дела не доходит. Первый: пока висит
  черновик правок, запись в активный конфиг молча откладывается — исчерпавший
  квоту (или истёкший) пир показан в панели приостановленным, но продолжает
  ходить в сеть до ближайшего Apply; интерактивные операции с квотой в это
  время отвечают 409 и прямо просят применить или отменить черновик. Второй:
  запись прошла, а перезагрузка amnezia-box (и запасной перезапуск) — нет.
  Конфиг на диске уже без пира, поэтому следующие тики не видят изменений и
  ничего не повторяют: пир остаётся живым до ближайшей перезагрузки или
  перезапуска amnezia-box, а единственный след — строка в логе
  `awg: singbox sweep sync`. У панельных пользователей отсрочка первого рода
  видна в поле `warning` ответа.

## [0.36.5]

### Changes

- **Главная: один график трафика с периодом, хост-метрики сбоку (#99).**
  Вместо четырёх равных полос — широкий график, где приём и отдача нарисованы
  друг над другом в одной шкале, с легендой кружками под ним и переключателем
  60 с / 1 ч / 24 ч. Минута по-прежнему живая из потока; час и сутки берутся
  из той же истории трафика, что и панель Breakdown (`/traffic/history` теперь
  умеет отдавать её временным рядом, `series=1`), пересчитанные в скорость и
  обновляемые раз в минуту. Справа узкая колонка: CPU с мини-графиком и память
  числом; в подвале тоталы и диск, RSS amnezia-box убран. Живая минута хранится
  вне страницы: уход в настройки и обратно не начинает график с нуля
  (заполняется он только пока главная открыта). Под графиком «среднее» словом
  целиком и рядом пик за выбранный период. Линия сглажена кривой через те же
  точки, без выхода за рамку на одиночном всплеске.

### Fixes

- **Импорт ссылки Hysteria2 доносит обфускацию и порты (#100).** Ссылка с
  `obfs=salamander&obfs-password=…` разбиралась верно, но форма затирала
  результат: блок обфускации хранил собственную копию состояния, снятую при
  открытии формы, и после импорта записывал поверх «пусто». Теперь блок правит
  саму настройку. Диапазоны для port hopping читаются из обеих форм записи —
  `mport=20000-50000` и hysteria'вской `srv:20000-50000` на месте порта — и
  попадают в поле «Порты сервера» (и в подписки). Выпадающий список
  «Отпечаток» у Hysteria2 убран: параметр `fp=` в ссылке — след v2rayN, а сам
  бинарник отказывается поднимать QUIC с uTLS-конфигом, так что сохранённый
  отпечаток не маскировал бы, а ломал outbound.

- **В группах соединений имя и адрес клиента снова одной фразой (#90).**
  Колонки фиксированной ширины выравнивали числа, но оставляли пустоту после
  коротких имён. Теперь имя с адресом читаются подряд, а счётчик соединений
  прижат к правому краю ячейки — числа совпадают между строками без зазоров.

## [0.36.4]

### Features

- **Живые полосы на главной.** Вместо двух карточек «Traffic Rate» и «Total
  Transfer» одна полоса из четырёх показателей: приём, отдача, CPU и память,
  у каждого крупное число и график за последнюю минуту. Трафик рисуется по
  тому же потоку, что и раньше, в одной шкале для приёма и отдачи, чтобы
  6 KB/s отдачи не выглядели ростом на 60 KB/s приёма. Хост-метрики читаются
  из /proc и statfs раз в две секунды: CPU как дельта между опросами, память
  как «использовано» в терминах `free`, load average, RSS процесса
  amnezia-box и диск `/`. В подвале полосы тоталы за сессию, RSS и диск.
  Точка API — `GET /api/system`. Направление выбрано по макетам из трёх
  вариантов.

- **Резервная копия сервера AmneziaWG (#97).** На странице AmneziaWG появился
  блок «Резервная копия»: экспорт отдаёт один JSON с настройками сервера,
  ключом сервера, ключом заголовков AWG3, префиксом IPv6 и всеми клиентами
  вместе с их секретами — тем, чего бэкап sing-box-конфига на вкладке Backup
  не содержал. Восстановление принимает этот файл на выключенном сервере,
  подменяет peers.toml и секцию awg настроек целиком, после чего остаётся
  нажать «Включить»: клиентские конфигурации продолжают работать, потому что
  ключ сервера тот же. Состояние (enabled/configured) и привязанный к хосту
  wan_iface в копию не входят. Точки API — `GET /api/awg/backup` и
  `POST /api/awg/restore`. Секреты в файле открытым текстом, как и в
  peers.toml.
...
- **«Rule sets with no route» больше не показывает наборы, у которых маршрут
  ...
  вторым в паре, или изнутри логического правила, — маршрут имеет; он значился
  под заголовком, утверждающим обратное, и предлагал корзину, на которую бэкенд
  честно отвечал `referenced by route rule[N]`. Теперь считается любое
  упоминание набора в любом правиле, а корзина блокируется и по маршрутным
  правилам, не только по DNS. По той же причине мастер больше не предлагает
  назначить набор, у которого маршрут уже есть.

## [0.36.3]

### Features

- **Замер скорости для endpoints (#92).** Кнопка-спидометр появилась в строке
  каждого endpoint'а: для AWG/WireGuard endpoint и есть outbound, и меряется он
  тем же инструментом. Точка API — `POST /api/endpoints/{tag}/speedtest`. Кнопка
  неактивна, пока endpoint существует только в черновике: замер идёт по
  применённому конфигу, и незаявленный тег там не найдётся.

### Changes

- **Ядерный AWG-сервер снова доступен там, где хост его тянет (#93).** Выбор
  бэкенда больше не зависит от режима установки: kernel предлагается везде, где
  система его потянет, а не только в vps-режиме. Заодно снят замкнутый круг:
  выбрать kernel не давали из-за отсутствия `amneziawg-tools`, а поставить их
  было некому — единственный установщик запускается как раз на kernel-бэкенде.
  Теперь отсутствие `awg-quick` ничего не запрещает там, где RouteBox может его
  поставить сам (Debian/Ubuntu, root, известный codename); где не может —
  причина названа целиком, обеими половинами.

  Заголовки ядра выбираются по запущенному ядру: на Proxmox VE это
  `pve-headers-*`, а не `linux-headers-*` — именно на этом установка падала, и
  из-за этого kernel-путь когда-то отключили целым режимам (Ubuntu-ВМ *внутри*
  Proxmox это никогда не касалось). Если пакета нет, в ошибке теперь его имя и
  что делать.

  В контейнере kernel-бэкенд по-прежнему не предлагается, если tools не входят
  в образ: ядро принадлежит хосту, и собирать DKMS не подо что — теперь это
  сказано сразу, а не выясняется на середине apt.

  Включение целиком переживает оборванный запрос: контекст отвязывается от
  HTTP-запроса на весь прогон, а не только на установку (потолок — 40 минут, из
  них 30 на сам apt). Раньше отмена после установки роняла генерацию ключей и
  подъём интерфейса с `context canceled` — над успешно поставленным модулем, — а
  отмена после подъёма забирала с собой ещё и откат. Пока идёт установка, панель
  показывает фазу («Ставим модуль ядра…») и состояние модуля вместо неподвижной
  кнопки, а шаг «Готовность» в мастере больше не рисует зелёную галочку над
  «ещё не установлен» и не путает установку с провалом.

- **Замер скорости переехал из Outbounds в Proxies (#90).** Меряют то, что
  сейчас работает, а смотрят на это в мониторе — там же, где задержка. Кнопка
  на карточке прокси помечена спидометром, чтобы не путать с молнией, которая
  меряет задержку. Группы (selector/urltest) меряются через выбранного
  участника; Reject/Block/DNS и синтетический `GLOBAL` кнопки не имеют — мерить
  нечего, а `GLOBAL` вообще не outbound, его рисует Clash API для своей панели.
  Замер идёт один за раз на всю страницу: пока считается одна карточка, кнопки
  остальных неактивны, а не отвечают ошибкой «уже идёт». Результат исчезает,
  если у группы сменили выбранного участника — он относился к прежнему пути.

### Fixes

- **Тест скорости больше не падает на хосте с AWG-сервером (#91).** Замер идёт
  по обрезанной копии конфига, но endpoints в ней оставались целиком — вместе с
  `listen_port`, который уже держит работающий процесс. Второй процесс садился
  на тот же порт и умирал с `IPC error -98 ... address already in use`, не
  измерив ничего. Теперь порт из копии убирается: клиентский endpoint
  дозванивается с эфемерного, а серверному за двенадцать секунд теста некого
  обслуживать. Вместе с портом убирается `system`: с ним измеряющий процесс
  поднял бы второй настоящий интерфейс ядра поверх работающего. Сами endpoints
  остаются — для AWG/WireGuard endpoint и есть outbound.

- **Правки по мелочам в интерфейсе (#90).** Коды возврата DNS («Fall back on»)
  переехали под выбор основного сервера, вместо отдельной строки под пустым
  местом; у кнопки «+ Add a server» убрано подчёркивание при наведении (осталось
  затемнение и курсор-указатель). В мониторе трафика приём теперь слева от
  отдачи — его смотрят первым. В списке соединений имя клиента, его адрес и
  число соединений разложены по колонкам фиксированной ширины: длинное имя
  обрезается с подсказкой по наведению, а не сдвигает соседние строки. Результат
  замера скорости подписан словами вместо `RPM 395 / 584`.

## [0.36.2]

### Features

- **Тест скорости через конкретный outbound (#13).** Кнопка-молния в списке
  Outbounds меряет приём, отдачу, задержку в покое и отзывчивость под нагрузкой
  (RPM) — по методике Apple, встроенной в сам amnezia-box, а не самодельным
  качанием файла. Рядом с каждой цифрой — точность, которую заявило само
  измерение: низкую не выдаём за результат. Тест идёт один за раз: два
  одновременных делят один канал и оба врут.

### Fixes

- **Панель больше не выдаёт «не подключался, 0 B» за измерение (#75).** Состояние
  каждого клиента AWG-сервера читается через отдельный маршрут amnezia-box,
  которого нет в сборках старше 19 августа; на них панель молча переходила на
  суррогат и рисовала нули как факт. Теперь каждая строка знает, откуда взяты её
  цифры: измеренные показываются как раньше, невыясненные — прочерком, а над
  списком написано, почему именно их нет. Причина отделена от самих цифр: старая
  сборка и не ответивший прокси выглядят одинаково, но советы им нужны
  противоположные, и панель больше не предлагает обновлять то, что и так свежее.
  Kernel-путь считался измеренным всегда — упавший `awg show` (удалённый
  интерфейс, пропавшая утилита) точно так же рисовал нули как факт; теперь и он
  честно говорит, что прочитать не удалось.

- **Видно, когда в цепочке записана только группа (#79).** Цепочку sing-box
  снимает в момент создания соединения и больше не пересчитывает, а url-test не
  сообщает выбранного участника, пока не прошла его первая проверка. Поэтому
  соединения, открытые сразу после старта или Apply, навсегда остаются
  подписанными одним именем группы — и в мониторе, и в истории трафика. Такие
  строки теперь помечены знаком вопроса с пояснением, вместо того чтобы
  выглядеть полноценным маршрутом.

## [0.36.1]

### Features

- **Автозапуск amnezia-box стал настройкой (#87).** Раньше RouteBox поднимал
  прокси при старте только в режиме панели; на роутере после каждой перезагрузки
  приходилось жать «Старт» вручную. Теперь это тумблер «Запускать amnezia-box
  при старте» в настройках (по умолчанию включён, оба режима). Прежние
  предохранители на месте: если процессом управляет systemd-юнит, если он уже
  запущен или если нет конфига либо самого бинарника — RouteBox не вмешивается.

- **Кнопка «Шлюз туннеля» у поля DNS AWG-сервера (#85).** Пустое поле по-прежнему
  означает «клиент оставит свой резолвер», но на роутере это уводит запросы мимо
  правил маршрутизации — и адрес, который это чинит, теперь ставится одним
  кликом вместо вычисления первого хоста подсети вручную.

- **Управление перегрузкой QUIC у naive-инбаунда (#73).** Факт подтверждён на
  выпущенном бинарнике: ключ `quic_congestion_control` у инбаунда есть, и набор
  значений у него шире клиентского — сервер умеет ещё `bbr_standard` и
  `bbr2_variant`. Все шесть теперь выбираются кнопками; по умолчанию, как и в
  бинарнике, BBR. Действует только на слушателя HTTP/3, TCP не касается.

### Fixes

- **Строки в списке правил маршрутизации стали почти вдвое ниже (#82).** Высоту
  задавали не сами стрелки «выше/ниже», а то, что они сложены в три этажа слева:
  на десктопе они теперь лежат рядом с грифом. Убирать их там было нельзя —
  drag&drop работает только мышью, так что стрелки остаются единственным
  способом переставить правило и с клавиатуры, и на сенсорном экране.
...
- **«Failed to fetch» на странице AmneziaWG (#78).** Страница обновляет статус
  раз в пять секунд, и любой сорвавшийся запрос — панель перезапускается после
  Apply, моргнул собственный туннель админа, ноутбук проснулся — выкидывал
  красный тост, и так по кругу. Фоновое обновление теперь молчит: вместо тоста в
  строке состояния появляется пометка, что цифры устарели, и она снимается сама,
  как только очередное обновление прошло. Заодно закрыт необработанный reject от
  обновления списка клиентов, случавшийся там же каждые пять секунд.

- **`::ffff:` вместо адреса клиента в мониторе (#71).** Инбаунды слушают
  двухстековый сокет, поэтому sing-box отдаёт IPv4-клиента в отображённом виде
  `::ffff:77.94.…`. Для панели это была другая строка, чем обычный адрес того же
  устройства: в списке клиентов заводилась вторая запись, которую надо было
  называть заново, история трафика делилась на две корзины, а монитор соединений
  показывал сырой `::ffff:` там, где ожидалось имя. Адрес приводится к обычному
  виду на входе — и в панели, и в истории, и в списке клиентов; уже сохранённые
  парные записи слипаются при старте, имя остаётся то, которое вы вводили.

- **Видно, какие outbound'ы принадлежат подписке (#80).** Подписка переписывает
  свои outbound'ы целиком при каждом обновлении, поэтому удаление вручную
  держится только до следующего — но в списке об этом ничего не говорилось, и
  удаление выглядело окончательным. Теперь у таких строк стоит бейдж с именем
  подписки, а подтверждение удаления честно предупреждает, что outbound
  вернётся.

- **Удаление набора правил прямо из «Rule sets with no route» (#86).** Удаление
  правила с назначенным outbound и раньше только снимало маршрут — набор падал
  в секцию ниже со значением «no route». Не хватало второй половины: убрать его
  оттуда совсем можно было только на отдельной странице Rule Sets. Теперь в
  строке есть корзина, и вопрос при удалении другой, чем у правила выше:
  снимается не маршрут, а сам набор.

- **История трафика тоже переезжает с `::ffff:` на обычный адрес.** Строки,
  записанные до исправления #71, оставались отдельной корзиной в Breakdown и не
  вычищались при удалении клиента. Теперь они сливаются с канонической при
  первом открытии базы, а совпавшие минуты складываются, а не теряются.

- **Резервных DNS теперь можно назначить несколько (#70).** Раньше запасной
  сервер был ровно один. Теперь это цепочка: серверы опрашиваются по порядку,
  каждый следующий — только если предыдущий ответил одним из выбранных кодов, и
  цепочка останавливается на первом, кто ответил иначе. Уже настроенный один
  резервный сервер читается как цепочка из одного звена и ничего не теряет.

- **Страница Endpoints больше не падает от числовых `h1`–`h4`.** sing-box
  принимает эти поля целыми числами, и только AWG 3.0 расширяет их до строки
  «lo-hi»; определение версии обфускации читало значение как строку и на
  импортированном или правленом руками конфиге роняло весь список эндпоинтов
  в вечное «Loading…».

- **Упавшая страница больше не уносит с собой всю панель.** Ошибка при отрисовке
  оставляла бесконечный спиннер, а в консоли — не относящееся к делу сообщение
  про локаль. Теперь сайдбар и шапка на месте, а на месте страницы написано,
