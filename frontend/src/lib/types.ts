...
export interface DnsConfig {
	...
	optimistic?: boolean;   // serve stale answers while refreshing (sing-box `optimistic`)
	final?: string;
...
export interface DnsRule {
	...
	query_type?: string[];
	rule_set?: string[];
	// Action ('evaluate'/'respond' only appear in the generated fallback tail, #68)
	action?: 'route' | 'reject' | 'predefined' | 'evaluate' | 'respond';
	server?: string;              // Required only for action: route
	// Response matching — generated by the DNS fallback switch, not editable here
...
// DnsFallback is not a sing-box dns.* field: it describes the generated tail of
